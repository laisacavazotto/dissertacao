...
Deliberadamente, os critérios de inclusão e exclusão não restringem a IHD a um domínio de aplicação específico (como o urbano) nem exigem menção literal aos três pilares fundacionais --- legibilidade, agência e negociabilidade --- propostos por \cite{mortier2015humandatainteractionhumanface}. Essa escolha é intencional, não uma lacuna do protocolo: um filtro fechado de domínio ou de vocabulário excluiria \textit{a priori} exatamente os estudos que esta revisão se propõe a identificar, isto é, aqueles que estendem, adaptam ou hibridizam os pilares fundacionais sob outra nomenclatura, ou que os aplicam em domínios não urbanos cujos achados dialogam por analogia com cidades inteligentes. Impor esse fechamento tornaria a pergunta de pesquisa autocumprida, mascarando a pluralidade terminológica de um campo interdisciplinar ainda em consolidação --- risco já reconhecido na discussão de validade de constructo desta revisão (Seção~\ref{sec:ameacas}). A leitura urbana dos achados é aplicada na etapa de síntese (Seção~\ref{sec:resultados}), a exemplo da hibridização da IHD com a Teoria Institucional, Sistemas Ciberfísicos e Inteligência Artificial Explicável discutida na Seção~\ref{sec:resultados}, e não como filtro de admissão ao \textit{corpus}.
